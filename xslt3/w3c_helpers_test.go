...
const (
	w3cTestdataDir = "../testdata/xslt30"
	...
	// XSLT-specific; XPath codes like XPTY0004 use "XPTY0004".
...
var (
	...
	w3cTestFileResolver = &w3cFileURIResolver{baseDir: string(filepath.Separator)}
...
func w3cCheckEmpty() w3cCheck {
	return w3cCheck{
		...
		rawFn: func(rawResult xpath3.Sequence) bool {
			return rawResult == nil || seqLen(rawResult) == 0
		},
	}
}
...
func w3cRunOne(t *testing.T, tc w3cTest) {
	...
	if outDef != nil && primaryItems != nil && seqLen(primaryItems) > 0 && (outDef.Method == "json" || outDef.Method == "adaptive" || buildTreeNo) {
...
func w3cImplicitSkipReason(name string) string {
	...
	if reason, ok := w3cExpectationSkips()[name]; ok {
		return reason
	}
	return ""
}
...
func castSequenceForParam(seq xpath3.Sequence, asType string) xpath3.Sequence {
	result := make(xpath3.ItemSlice, seqLen(seq))
...
func (r *w3cCollectionResolver) ResolveCollection(uri string) (xpath3.Sequence, error) {
	...
	return xpath3.ItemSlice(append([]xpath3.Item(nil), seqMaterialize(seq)...)), nil
}
